...
if(isset($_GET['ids'])){
    ...
    header("refresh:2;url=manage-category.php?id=".$_GET['id']);  
		?>
...
if(isset($_GET['tag']) && $_GET['tag']=='ProgarmActivateDeactivate')
{
   $query= $user->updateStatus($table,"status",$_GET['active'],$_GET['id']);
	...
	$query = "select * from $table where userId = '".$userId."' order by id desc";
              ...
              <div class="box">
                <div class="box-header">
                  ...
                  <div class="pull-right">
                    <!-- <button type="submit" onclick="window.location.href='view-note.php?action=add'" id="button-invoice" form="form-order" formaction="#" formtarget="_blank" data-toggle="tooltip" title="" class="btn btn-primary" data-original-title="Add"><i class="fa fa-plus-circle"></i></button> -->
                  </div>
               ...
               <table id="example1" class="table table-bordered table-striped">
					...
					<?php while($x < $arr){  
            $cat = $user->getResultById('category_list', $results[$x]['catId']);
            ?>
				<tr>				
            <td><?php echo $i; ?></td>
             <td>  <?php echo $results[$x]['title']; ?></td>
             <td>  <?php echo $cat['catName']; ?></td>
            <td> <?php echo (($results[$x]['status']=='1')?'Active':'Trash') ?>
                      <!-- <form method="POST" class="toggle-form">
						...
						<td>
              <a href="view-note.php?edit=<?php echo $results[$x]['id'];?>"><span class="btn btn-success btn-xs"><i class="fa fa-edit"></i>Edit</span></a>
			
               <a href="note_list.php?ids=<?php echo $results[$x]['id'];?>&id=<?php echo $_GET['id']?>" class="btn btn-danger btn-xs" onClick="return confirm('Are you sure want to delete')">Delete</a>
